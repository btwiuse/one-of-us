...
export class PostgresDatabase implements DatabaseProvider {
  ...
  async isMember(address: string): Promise<boolean> {
    const normalizedAddress = address.toLowerCase();
    const result = await this.pool.query('SELECT 1 FROM members WHERE address = $1', [
      normalizedAddress,
    ]);
  ...
  async getMember(address: string): Promise<Member | null> {
    const normalizedAddress = address.toLowerCase();
    const result = await this.pool.query('SELECT * FROM members WHERE address = $1', [
      normalizedAddress,
    ]);
  ...
  async getMemberCount(): Promise<number> {
    const result = await this.pool.query('SELECT COUNT(*) as count FROM members');
